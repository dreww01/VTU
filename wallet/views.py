...
def verify_payment(request, reference):
    ...
    try:
        ...
        logger.info("Paystack verification response for ref %s: status=%s", reference, data.get('status'))

        # Check if verification was successful
        if data['status'] and data['data']['status'] == 'success':
            amount = Decimal(data['data']['amount']) / 100  # Convert kobo to naira
            logger.info("Payment verified: ref=%s, amount=N%s, user=%s", reference, amount, request.user.username)
            ...
            with transaction.atomic():
                ...
                if Transaction.objects.filter(reference=reference).exists():
                    logger.info("Duplicate payment attempt: ref=%s, user=%s", reference, request.user.username)
                ...
                logger.info("Wallet credited: user=%s, amount=N%s, new_balance=N%s",
                           request.user.username, amount, wallet.balance)
        ...
        logger.error("Paystack connection timeout: ref=%s, user=%s", reference, request.user.username)
        ...
        logger.error("Paystack network error: ref=%s, user=%s, error=%s", reference, request.user.username, str(e))
        ...
        logger.exception("Unexpected error during payment verification: ref=%s, user=%s", reference, request.user.username)
...
def paystack_webhook(request):
    ...
    if signature != computed_sig:
        logger.warning(f"Invalid Paystack webhook signature from IP: {request.META.get('REMOTE_ADDR')}")
