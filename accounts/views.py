...
def register_view(request):
    ...
    if getattr(request, "limited", False):
        logger.warning(
            f"Rate limit exceeded for registration from IP: {request.META.get('REMOTE_ADDR')}"
        )
        messages.error(
            request, "Too many registration attempts. Please wait a minute before trying again."
        )
...
def login_view(request):
    ...
    if getattr(request, "limited", False):
        ...
        messages.error(
            request, "Too many login attempts. Please wait a minute before trying again."
        )
...
def dashboard_view(request):
    ...
    try:
        ...
        if wallet.balance is None or not isinstance(wallet.balance, (int, float, Decimal)):
            ...
            logger.error(
                f"Invalid or None balance for user {request.user.username}, resetting to ₦0.00"
            )
        ...
        try:
            ...
            logger.error(
                f"Invalid balance value for user {request.user.username}: {wallet.balance}"
            )
...
def send_reset_code(request):
    ...
    if getattr(request, "limited", False):
        logger.warning(
            f"Rate limit exceeded for password reset from IP: {request.META.get('REMOTE_ADDR')}"
        )
        messages.error(
            request, "Too many password reset requests. Please wait a minute before trying again."
        )
    ...
    if request.method == "POST":
        ...
        if "reset_code" in request.POST:
            ...
            if not entered_code.isdigit() or len(entered_code) != 6:
                logger.warning(f"Invalid reset code format for email {email}: {entered_code}")
                return render(
                    request,
        ...
        # ── PHASE 2: SET NEW PASSWORD ───────────────────────────
            ...
            if not request.session.get("code_verified"):
                ...
                logger.warning(f"Password set attempted without code verification for: {email}")
                return redirect("password_reset_confirm")
