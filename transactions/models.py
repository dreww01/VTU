...
class Transaction(models.Model):
    ...
    description = models.CharField(
        max_length=255, help_text="Description (e.g., 'Paystack Deposit', 'MTN Airtime')"
    )
    ...
    provider_response = models.JSONField(
        null=True, blank=True, help_text="Last raw provider response for support/debugging."
    )
    requires_manual_review = models.BooleanField(
        default=False, help_text="Flag for customer support to manually review this transaction."
    )
    ...
    def save(self, *args, **kwargs):
        ...
        logger.info(
            f"Transaction saved: {self.transaction_type} {self.amount} NGN for {self.wallet.user.username}"
        )
...
class AppSettings(models.Model):
    ...
    maintenance_mode = models.BooleanField(
        default=False, help_text="Put the app in maintenance mode (blocks all transactions)"
    )
